...
module.exports = class {
    ...
    async run(member) {
        ...
        if (action.target.id !== member.user.id) return;
        ...
        if (client.config.owners.includes(action.executor.id) || guild.owner.id == action.executor.id || guildData.whitelist.includes(action.executor.id) || !guildData.guildKickAdd) {
            let logChannel = client.guilds.cache.get(guild.id).channels.cache.get(guildData.protectLog)

            if (logChannel) logChannel.send(client.translator(guildData.language).event("MEMBER_KICK", {
                ...member,
                ...action,
                timeout: Date.now() - startAt,
                triggered: false
            }))
        } else if (guildData.guildKickAdd && !client.config.owners.includes(action.executor.id) || guild.owner.id !== action.executor.id) {
            let logChannel = client.guilds.cache.get(guild.id).channels.cache.get(guildData.protectLog)
